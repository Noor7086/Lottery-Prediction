...
4. **Use this URL** as your backend URL
...
1. **Create a `.env.production` file** in the root directory:
   ...
   - Your localtunnel URL (if using localtunnel)
   - Your public IP:port (if using port forwarding)
...
3. **Upload the `dist` folder** to your Hostinger hosting
...
```env
CORS_ORIGINS=https://seagreen-owl-810079.hostingersite.com,https://abc123.ngrok.io,http://localhost:3000
```
...
- For ngrok/localtunnel: Make sure the tunnel is active
...
1. **For Development**: Using ngrok/localtunnel is fine
